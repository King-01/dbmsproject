...
<body>
	...
	<c:forEach var="tax" items="${list}">
		<c:url var="taxlink" value="/view/taxbyid">
			<c:param name="taxId" value="${tax.getId()}" />
		</c:url><br>
		<number><a href="${taxlink}"><c:out value="${tax.getTaxName()}"/></a></number>
		
	</c:forEach>
	</startcounter>
	<form:form action="${pageContext.request.contextPath}/add/tax" 
					  	   >
